    ...
    public void request(Request r)throws IOException {
        ...
        try{
            switch (r.getRequestMethod()){
                case GET: response = doGet(r);break;
                case POST: response = doPost(r);break;
            }
        }catch (IOException e){
            throw e;
        }catch (Exception e){
        ...
        if((tempMap = request.getParamMap()) != null){
            ...
            switch (contentType){
                ...
                case MULTIPART_FORM_DATA:
                    ...
                    String boundary = "-----FormBoundary" + CommonStringUtils.getRandomString(16);
                    ...
                    break;
                default:
                    httpEntity = new StringEntity(CommonStringUtils.emptyString);
            }
    ...
    private static Response buildResponse(HttpResponse srcResponse,Request request,HttpClientContext context) throws IOException{
        ...
        for(Header header : headers){
            ...
            if("Host".equals(header.getName())){
                ...
                //获取字符集
                String temp = CommonStringUtils.getFirstMatch(header.getValue().toLowerCase(),"charset=\\s*[\\w-]+");
                destResponse.setCharSet(CommonStringUtils.isEmptyOrWihtespace(temp) ? temp : temp.substring(8).trim());
            }
        }
        if(destResponse.getDomain() == null){
            //如果无法从http头中获取到域名，则用正则表达式从url中抠出域名
            destResponse.setDomain(CommonUrlUtils.getDomain(request.getRequestURL()));
        }
        if(destResponse.getDomain() == null){
            //如果无法从http头中获取到字符集，则set空字符串，使用默认字符集
            destResponse.setCharSet(CommonStringUtils.emptyString);
        }
